...
$stageRoot = Join-Path $releaseRoot "Blackbox-$version"
$zipPath = Join-Path $releaseRoot "blackbox-$version-windows-x64.zip"
$cacheRoot = Join-Path $repoRoot ".blackbox-cache"
$nodeVersion = "24.13.1"
...
if ($LASTEXITCODE -ne 0) { throw "Blackbox release build failed." }
