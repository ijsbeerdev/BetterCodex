...
Keep the injected UI in `src/client.js`, the launcher and CDP transport in `src/launcher.mjs` and `src/cdp.mjs`, and the normal-launch watcher in `src/watcher.ps1`. Put add-ons under `addons/<id>/`, tweaks under `tweaks/<id>/`, and themes under `themes/<id>/`. Machine-level installation changes belong in `scripts/patch.ps1` and `scripts/unpatch.ps1` and must remain reversible.
